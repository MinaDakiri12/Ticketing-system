...
// Add items data in cart table
const cardBoxTable = document.querySelector('.cart-table')
let tableData = ''
tableData +=  `<tr>
                    <th>ID</th>
                    <th>Product Name</th>
...
if(JSON.parse(localStorage.getItem('items'))[0] === null){
     ...
     JSON.parse(localStorage.getItem('items')).map(data=>{
          tableData +=  `<tr>
                              ...
                              <td>${data.qty}</td>
                              <td>${data.price}</td>
                              <td><button class="btn btn-sm btn-danger" onclick=Delete(this);> <i class="far fa-trash-alt"></i> </button></td>
                         </tr>`
     });
}
...
let totalPriceNum = 0;
JSON.parse(localStorage.getItem('items')).map(data=>{
     totalPriceNum = Number(totalPriceNum) + Number(data.price)
})
...
if(JSON.parse(localStorage.getItem('items'))[0] === null){
     ...
     JSON.parse(localStorage.getItem('items')).map(data=>{
          listData +=   `<li class="list-group-item d-flex justify-content-between lh-condensed">
                              <div>
                                   <h6 class="my-0">${data.name}</h6>
                                   <small class="text-muted">Quantity: ${data.qty}</small>
                              </div>
